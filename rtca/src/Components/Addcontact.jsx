...
//handle click function to hide the add contact box
  ...
  return (
    <div className='flex justify-center  flex-col'>
      {popup && <div className='fixed flex flex-col gap-2 items-center w-full mt-5'>
        {popupmess && <Alert className='w-190' variant="filled" severity="success">
          {popupmess}

        </Alert>}
        {popupmessErr &&
          <Alert className='w-190' variant="filled" severity="error">
            {popupmessErr}
          </Alert>}
           {popupmessinfo &&
          <Alert className='w-190' variant="filled" severity="warning">
            {popupmessinfo}
          </Alert>}
      </div>}
      ...
      {<div id='add-contact' className='fixed block  top-[30%] left-[36%] border-[1px]  w-120 h-90 rounded-3xl backdrop-blur-xs backdrop-grayscale drop-shadow-lg z-50'>
        ...
        <div className='flex flex-col items-start h-full gap-2 p-5  '>
          ...
          <div className='flex gap-5 self-end mt-5 mx-10'>
            <button className=' flex gap-2 content-center bg-red-500 px-4 py-2 rounded-[7px]' onClick={() => handleClick()}>cancel  </button>
